...
% and the number within own district and own cohort. 
...
We estimated the ``school regime" coefficients $\nu,\dots,\rho^\star$ by two variants of sub-cohort comparison (if applicable) and two variants of direct estimation (see \supfile Section \ref{sec:methodsdetail} for details); consequently, we made meta-estimates out of all the available estimates. The meta-estimates are computed as weighted averages of the coefficients resulting from the available sub-cohort comparisons and the direct estimations with strong preference of the former (see \supfile Section \ref{sec:res} for details). To distinguish the quality of the meta-estimates, we have split them into the following four categories. 
\begin{enumerate}
    \item[\ratei] At least one source estimate comes from a comparison of a regime and the closure, where both the cohorts correspond to the same school level and are "interior" meaning that they are not shared with another shool level or sub-cohort, e.g. 7-year-olds (Grade 1/2 open) vs. 9-year-olds (Grade 3/4 closed). 
    \item[\rateii] At least one source estimate is a comparison of a regime and the closure such that either the comparison includes cohorts from different school levels (e.g. 5-year-olds [kindergarten open] vs. 7-year-olds  [Grade1/2 closed]) or a "boundary" cohort (i.e. only the last grade of some school level was opened while the rest was closed). In the latter case, because no age cohort existed that would include only individuals from the open grade, halves of the boundary cohorts had to be used. 
    \item[\rateiii] At least one source estimate is a sub-cohort comparison, but of two different regimes (e.g. masks and rotations). 
